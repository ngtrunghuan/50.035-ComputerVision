...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # the out variable.                                                         #
        #############################################################################
        pasx_hat = (x - running_mean) / np.sqrt(running_var)
        out = x_hat * gamma + betas
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    else:
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N, C, Hx, Wx = x.shape
    F, _, Hw, Ww = w.shape
    H1 = int(1 + (Hx + 2 * pad - Hw) / stride)
    W1 = int(1 + (Wx + 2 * pad - Ww) / stride)
    ...
    for n in range(N):
        for f in range(F):
            for h1 in range(H1):
                for w1 in range(W1):
                    out[n, f, h1, w1] = np.sum(x_pad[n, :, h1*stride : h1*stride+Hw, w1*stride : w1*stride+Ww] * w[f,:,:,:]) + b[f]   -1
...
def conv_backward_naive(dout, cache):
    ...
    for f in range(F):
        for c in range(C):
            for h1 in range(HH): 
                for w1 in range(WW):
                    dw[f, c, h1, w1] = np.sum(dout[:, f, :, :] * x_pad[:, c, h1:h1 + H_prime * stride:stride, w1:w1 + W_prime * stride:stride])
                        ...
                        for ww in range(WW):
                            for hp in range(H_prime):
                                for wp in range(W_prime):
                                    if (hp * stride + hh - pad == h1) and (wp * stride + ww - pad == w1):
                                        dx[n, c, h1, w1] += np.sum(dout[n, :, hp, wp] * w[:,c,hh,ww])
